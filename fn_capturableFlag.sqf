...
[
    ...
    "Capture flag",
    "", "",
    "_this distance _target < 3",   // "(_this distance _target < 3) && ([side _this] call BIS_fnc_sideID != _target getVariable 'flagPoleSideOwner')",
    "_caller distance _target < 3", // "_caller distance _target < 3 && ([side _caller] call BIS_fnc_sideID != _target getVariable 'flagPoleSideOwner')",
    {},
    _fn_codeProgress,
    _fn_codeCompleted,
    {},
